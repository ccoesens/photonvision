...
import BarcodeTab from "./tabs/BarcodeTab.vue";
...
const getTabGroups = (): ConfigOption[][] => {
  ...
  if (smAndDown || useCameraSettingsStore().isDriverMode || (mdAndDown && !useStateStore().sidebarFolded)) {
    ...
    return [
      [allTabs.inputTab],
      [allTabs.thresholdTab],
      [allTabs.contoursTab, allTabs.apriltagTab, allTabs.barcodeTab, allTabs.arucoTab, allTabs.objectDetectionTab, allTabs.outputTab],
      [allTabs.targetsTab, allTabs.pnpTab, allTabs.map3dTab]
    ];
  }
...
const tabGroups = computed<ConfigOption[][]>(() => {
  ...
  const isBarcode = useCameraSettingsStore().currentWebsocketPipelineType === WebsocketPipelineType.Barcode;
    ...
    .map((tabGroup) =>
      tabGroup.filter(
        (tabConfig) =>
          ...
          !((isAprilTag || isAruco || isObjectDetection || isBarcode) && tabConfig.tabName === "Threshold") && //Filter out threshold tab if we're doing AprilTags
          !((isAprilTag || isAruco || isObjectDetection || isBarcode) && tabConfig.tabName === "Contours") && //Filter out contours if we're doing AprilTags
          ...
          !(!isBarcode && tabConfig.tabName === "Barcode") //Filter out barcode unless we actually are doing Barcode
      )
    )
